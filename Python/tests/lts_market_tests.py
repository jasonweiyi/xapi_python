...
from base.comm import Disconnected
...
class LtsMarketTests(unittest.TestCase):
    ...
    def test_depth_market_data_single(self):
        ...
        self.process(stop_condition=lambda: len(self.callbacks.market_data.get(self.instrument_ids[0], [])) > 10)
        self.lts.unsubscribe(self.instrument_ids[0])
        self.assertNotEqual(len(self.callbacks.market_data.get(self.instrument_ids[0], [])), 0, msg='Get Depth Market Data Failed!')
    ...
    def test_depth_market_data_multi(self):
        ...
        for ins in self.instrument_ids[:5]:
            self.assertNotEqual(len(self.callbacks.market_data.get(ins, [])), 0, msg='Get Depth Market Data for %s Failed!' % ins)
